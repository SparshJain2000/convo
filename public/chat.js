...
button.addEventListener("click", () => {
  ...
  addMessage = async () => {
    output.innerHTML +=
      `<div style=${style} ><div class='${bg}'><h6 class= ${color}>${data.handle}</h6><div>${data.message}</div><img class="img-fluid rounded mb-2" src='${data.image}'/><div style="text-align:right;font-size:2vmin"> 
    ${
...
socket.on("joined", (data) => {
  ...
  document.querySelector("#roomInfo").innerHTML = `<div >
                                                        ...
                                                        <a href="whatsapp://send?text=${data.room.roomName}" data-action="share/whatsapp/share" class='btn btn-primary' onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on whatsapp"  data-toggle="tooltip" data-placement="bottom"><i class='fa fa-share'></i></a>
...
socket.on("chat", (data) => {
  ...
  if (data.handle === name.textContent) {
    ...
    if (users !== "") {
      output.innerHTML +=
        `<div style=${style}><div class='bg-success seen pl-2 pr-2 p-1 mr-2 rounded col-8 '><strong><em>Seen by ${users} </em></strong></div></div>`;
    }
    $(".spinner-border").parent().append(
