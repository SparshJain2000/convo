...
button.addEventListener('click', () => {
	...
	addMessage = async () => {
		output.innerHTML += `<div style=${style} ><div class='${bg}'><h6 class= ${color}>${data.handle}</h6><div>${data.message}</div><img class="img-fluid rounded mb-2" src='${data.image}'/><div style="text-align:right;font-size:2vmin"> 
...
socket.on('joined', (data) => {
	...
	document.querySelector('#roomInfo').innerHTML = `<div >
                                                        <button type="button" class="btn btn-secondary"  data-toggle="tooltip" data-html="true" title="Copy <b>Room-id</b>" data-placement="left" onclick='copyId("${data
															.room.roomName}")' id='tooltip'>
                                                            RoomId: <em class='text-warning'>${data.room
																.roomName}</em>&nbsp <strong>|</strong>&nbsp
                                                            Admin : <em class='text-warning'>${data.room.admin}</em>
                                                        </button>
                                                        <a href="whatsapp://send?text=${data.room
															.roomName}" data-action="share/whatsapp/share" class='btn btn-primary' onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on whatsapp"  data-toggle="tooltip" data-placement="bottom"><i class='fa fa-share'></i></a>
...
socket.on('invalidRoom', ({ message }) => {
	document.querySelector('.close').click();
	$('#alert').html("<div class='alert alert-warning' role='alert'>" + message + '</div>').hide();
	$('#alert').slideDown(500);
	window.setTimeout(function () {
		$('.alert').fadeTo(500, 0).slideUp(500, function () {
			$(this).remove();
...
socket.on('typing', (data) => {
	...
	else $('#feedback').html(`<p class='badge badge-info p-2 ml-2 type'><em>${data} is typing .... </em></p>`);
	scroll();
});
...
socket.on('userDisconnected', (data) => {
	$('#alert').html("<div class='alert alert-danger' role='alert'>" + data + ' has left the chat' + '</div>').hide();
	$('#alert').slideDown(500);
	window.setTimeout(function () {
		$('.alert').fadeTo(500, 0).slideUp(500, function () {
			$(this).remove();
